...
class Data_Transformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        y = self.add_Half_Life_Seconds(y)
    ...
    def convert_to_seconds(self, time):
        ...
        if(mes == 'AS'):
            ...
            return self.convert_to_seconds('%sMS'%(value/a))
        
        if(mes == 'P'):
            return self.convert_to_seconds('%sN'%(value/1000))
        if(mes == 'N'):
            return self.convert_to_seconds('%sU'%(value/1000))
        if(mes == 'U'):
            return self.convert_to_seconds('%sMS'%(value/1000))
        if(mes == 'MS'):
            return self.convert_to_seconds('%sS'%(value/1000))
        
        if(mes == 'M'):
            return self.convert_to_seconds('%sS'%(value*60))
        if(mes == 'H'):
            return self.convert_to_seconds('%sM'%(value*60))
        if(mes == 'D'):
            return self.convert_to_seconds('%sH'%(value*24))
        if(mes == 'Y'):
            return self.convert_to_seconds('%sD'%(value*365))
    ...
    def add_Half_Life_Seconds(self, y):
        y['Half Life (Seconds)'] = [self.convert_to_seconds(n) for n in y['Half Life']]
        return y
    ...
    def get_isStable_feature(self, y, magnitude_threshold, seconds_threshold:float):
        ...
        y.loc[y['Half Life'] != y['Half Life'], 'isStable'] = np.nan
